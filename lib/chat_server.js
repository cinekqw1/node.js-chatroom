...
function joinRoom(socket, room) {
  ...
  if(usersInRoom.length>1){
    ...
    for(var index in usersInRoom[index]){
      var userSocketId=usersInRoom[index].id;
      if(userSocketId!=socked.id){
        if(index>0){
          usersInRoomSummary+='. ';
        }
  ...
  socket.on('nameAttempt', function(name){
    if(name.idexOf('Gość')==0){
      ...
      if(namesUsed.indexOf(name)==-1){
        var previosName=nickNames[socket.id];
        var previosNameIndex=namesUsed.indexOf(previosName);
        namesUsed.push(name);
        nickNames[socket.id]=name;
        delete namesUsed[previosNameIndex];
        socket.emit('nameResult',{
          success: true,
          name: name
        });
        socket.broadcast.to(currentRoom[socket.id]).emit('message',{
          text: previosName + ' zmienił nazwę na '+ name + ' .'
        });
      }else{
        socket.emit('nameResult',{
          success: false,
